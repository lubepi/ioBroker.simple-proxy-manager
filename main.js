...
class SimpleProxyManager extends utils.Adapter {
  ...
  resolveCertCollection(collectionName, certsObj) {
    ...
    const namedCert =
      certificates[`${collectionName}Chained`] ||
      certificates[`${collectionName}Public`];
  ...
  async loadAllCertificates() {
    try {
      ...
      if (!defaultSslOptions) {
        this.log.error(
          "All configured certificate collections failed to load – HTTPS server cannot start",
        );
        return null;
      }
    ...
    this.httpsServer = https.createServer(
      {
        ...
        SNICallback: (servername, cb) => {
          ...
          if (ctx) {
            ...
            this.log.debug(
              `TLS handshake rejected for unknown host: ${servername}`,
            );
  ...
  async onMessage(obj) {
    ...
    if (obj.command === "getCertificateCollections") {
      try {
        const certsObj = await this.getForeignObjectAsync(
          "system.certificates",
        );
