...
class SimpleProxyManager extends utils.Adapter {
            ...
            for (const entry of config.backends) {
                ...
                if (entry.allowedNetworks) {
                    if (typeof entry.allowedNetworks !== 'string') {
                        ...
                        this.terminate(`Invalid configuration: allowedNetworks type for ${entry.hostname}`);
                        return;
                    }
     ...
     *
     * @param collectionName - Name of the certificate collection to resolve
     * @param certsObj - The system.certificates object from ioBroker
     */
    ...
    async loadAllCertificates() {
        try {
            ...
            this.log.debug(`Available certificate collections: ${availableCollections.join(', ')}`);
            ...
            for (const backend of Object.values(this.backends)) {
                if (backend.certificate) {
                    usedCollections.add(backend.certificate);
                }
            }
            ...
            for (const channel of existingChannels || []) {
                ...
                if (!usedCollections.has(certName)) {
                    try {
                        ...
                        this.log.debug(`Could not remove stale state certificates.${certName}.expires: ${e.message}`);
                    }
                    try {
                        await this.delObjectAsync(`certificates.${certName}.daysLeft`);
                    } catch (e) {
                        this.log.debug(`Could not remove stale state certificates.${certName}.daysLeft: ${e.message}`);
                    }
                    try {
                        await this.delObjectAsync(`certificates.${certName}`);
                    } catch (e) {
                        this.log.debug(`Could not remove stale channel certificates.${certName}: ${e.message}`);
                    }
            ...
            for (const collName of usedCollections) {
                ...
                if (!resolved) {
                    ...
                    if (affectedHosts.length > 0) {
                        this.log.error(`HTTPS disabled for host(s): ${affectedHosts.join(', ')}`);
                    }
                    continue;
                }
                ...
                try {
                    ...
                    const affectedHosts = this.markInvalidCertificateHosts(collName);
                    this.log.error(`Failed to create TLS context for "${collName}": ${tlsErr.message}`);
                    if (affectedHosts.length > 0) {
                        this.log.error(`HTTPS disabled for host(s): ${affectedHosts.join(', ')}`);
                    }
                    continue;
                ...
                if (expiryDate && !isNaN(expiryDate.getTime())) {
                    ...
                    await this.setStateAsync(`certificates.${collName}.expires`, expiryDate.toISOString(), true);
                    ...
                    this.log.debug(`Certificate "${collName}" loaded (expiry date unknown)`);
                }
            }
    ...
    async checkCertificateRenewal() {
        try {
            ...
            for (const backend of Object.values(this.backends)) {
                if (backend.certificate) {
                    usedCertNames.add(backend.certificate);
                }
            }
            ...
            for (const collName of usedCertNames) {
                try {
                    ...
                    this.log.error(`Error while processing certificate "${collName}": ${e.message}`);
                }
            }
    ...
    clearInvalidCertificateHosts(collectionName) {
        ...
        for (const [hostname, backend] of Object.entries(this.backends)) {
            if (backend.certificate === collectionName && this.invalidCertHosts.has(hostname)) {
    ...
    hasReadyCertificateForHost(hostname) {
        return !!(hostname && this.certContexts[hostname] && !this.invalidCertHosts.has(hostname));
    }
    ...
    updateConnectionState() {
        try {
            this.setState('info.connection', this.httpsListening && this.httpListening, true);
        } catch (e) {
     ...
     *
     * @param collName - Name of the certificate collection
     */
    ...
    static parseIP(ip) {
        ...
        if (ip.includes(':')) {
            return SimpleProxyManager.parseIPv6(ip);
        }
    ...
    static ipMatchesCIDR(ipBytes, network) {
        for (let i = 0; i < 16; i++) {
            ...
            if ((ipBytes[i] & mask) !== (network.bytes[i] & mask)) {
                return false;
            }
        }
        return true;
    ...
    isAllowedIP(clientIP, backend) {
        ...
        for (const network of networks) {
            if (SimpleProxyManager.ipMatchesCIDR(ipBytes, network)) {
                return true;
            }
        }
        return false;
     ...
     *
     * @param req - The incoming HTTPS request
     ...
     * @param socket - The network socket between server and client
    ...
    handleUpgrade(req, socket, head) {
        try {
            ...
            if (this.config.logRequests) {
                this.log.debug(`${clientIP} -> WS${isHttps ? 'S' : ''} ${host}${req.url}`);
            }
            ...
            if (isHttps && backend.certificate && !this.hasReadyCertificateForHost(host)) {
                socket.destroy();
                return;
            }
    ...
    startProxy(sslOptions) {
        ...
        this.proxy.on('error', (err, req, res) => {
            try {
                ...
                const message = handlerError && handlerError.message ? handlerError.message : String(handlerError);
        ...
        if (this.hstsHeader) {
            this.proxy.on('proxyRes', (proxyRes, req) => {
                try {
                    ...
                    if (req && req.socket && req.socket.encrypted && proxyRes && proxyRes.headers) {
        ...
        for (const [host, cfg] of Object.entries(this.backends)) {
            ...
            const netInfo =
                cfg.allowedNetworks.length > 0 ? ` (networks: ${cfg.allowedNetworks.join(', ')})` : ' (all IPs)';
            this.log.debug(`  ${host} -> ${cfg.target} [${proto}]${certInfo}${netInfo}`);
        }
        ...
        this.log.debug(`Certificate check interval: every ${config.certCheckHours || 1} hour(s)`);
    }
     ...
     *
     * @param obj - The message object from the admin UI
     */
    async onMessage(obj) {
        if (!obj || !obj.command) {
            return;
        ...
        if (obj.command === 'getCertificateCollections') {
            try {
                const certsObj = await this.getForeignObjectAsync('system.certificates');
                const result = [{ value: '', label: '(no certificate)' }];
