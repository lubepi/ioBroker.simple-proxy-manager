...
This adapter is **ioBroker.simple-proxy-manager** — a Simple HTTPS/HTTP Reverse Proxy Manager for ioBroker. It acts as a virtual-host aware reverse proxy that routes incoming HTTP/HTTPS traffic to internal backend services based on the `Host` header.

**Adapter-Specific Context:**
- **Adapter Name:** ioBroker.simple-proxy-manager
- **Primary Function:** HTTPS/HTTP reverse proxy with per-virtual-host routing, SNI-based TLS, IP filtering (CIDR), HSTS, WebSocket support, and automatic SSL certificate reload via the ioBroker ACME adapter.
- **Key Dependencies:** `http-proxy` (npm) for proxying, Node.js built-ins `https`, `http`, `tls`, `crypto`; `@iobroker/adapter-core` for adapter base.
- **Configuration Requirements (io-package.json):** Backend list (hostname, target URL, enabled flag, certificate collection name, IP allowlist), HTTPS/HTTP ports, ACME adapter port, HSTS settings, certificate check interval and expiry-warning threshold.
- **No external API authentication required** — the adapter proxies local or network-reachable backend URLs configured by the user.
- **Certificate handling:** TLS contexts are built per virtual host using `tls.createSecureContext()`. Certificate collections are read from ioBroker objects (ACME adapter or manual). Change detection via SHA-256 hash avoids unnecessary context rebuilds.
- **Dual-stack:** Both IPv4 and IPv6 are supported. IP filtering uses CIDR matching for both address families.
...
* (lubepi) **FIXED**: Adapter now properly validates backend hostname configuration (fixes #25)
* (lubepi) **NEW**: Added IP filtering (CIDR) for internal backend services
```
...
- The proxy core (`http-proxy`) is started once during `onReady()` and torn down in `onUnload()`. Do not re-create the proxy instance on configuration changes — restart the adapter instead.
- TLS contexts are stored in `this.certContexts` (keyed by hostname). Always use `tls.createSecureContext()` and supply contexts via the `SNICallback` on the HTTPS server.
- Certificate collection objects in ioBroker follow the path pattern `<acmeAdapterNamespace>.collections.<collectionName>`. When reading certs, handle the case where the object does not yet exist (ACME not yet issued).
- IP allowlists use CIDR notation (e.g., `192.168.0.0/24`, `::1/128`). Use a CIDR-matching utility or implement manually with bitwise ops — do not use string `startsWith` for IP filtering.
- WebSocket proxying requires passing `{ws: true}` to `httpProxy.createProxyServer()` and upgrading the connection via `server.on('upgrade', ...)`.
- Always set `this.setState('info.connection', ...)` to reflect adapter connectivity status.
- Log security events (denied IPs, invalid certs) at `warn` level when `logSecurityEvents` config option is enabled, otherwise suppress.
